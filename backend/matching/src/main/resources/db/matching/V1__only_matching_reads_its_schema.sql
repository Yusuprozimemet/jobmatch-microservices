-- Day 38: only matching reads schema matching. V14:17 in app granted SELECT on its tables to the
-- other module logins, and the role sources granted them the schema and whatever matching_user
-- created here later. Before matching leaves the process, its data stops being readable by logins
...
-- rather than from a list of names the next role added would be missing from. matching_user
...
BEGIN
    ...
    -- What matching_user creates here later. Defaults registered for other creators (db-setup.py
    ...
    FOR target IN
        ...
        WHERE d.defaclnamespace = 'matching'::regnamespace AND d.defaclrole = current_user::regrole
          AND acl.grantee <> d.defaclrole
    LOOP
        EXECUTE 'ALTER DEFAULT PRIVILEGES IN SCHEMA matching REVOKE ALL ON ' || target.kind || ' FROM ' || target.grantee;
    END LOOP;
END
$$;
